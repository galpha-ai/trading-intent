...
\textbf{本论文是公开仓库的 white paper：} \href{https://github.com/galpha-ai/trading-intent}{github.com/galpha-ai/trading-intent}。本文专门用于解释该仓库当前实现的 semantic contract layer，以及它所指向但尚未在仓库中完整实现的 execution hierarchy。
}}
\end{center}
...
Trade intent schema 正是贯穿这些阶段的稳定接口。配套的 GitHub 仓库 \texttt{galpha-ai/trading-intent}（\href{https://github.com/galpha-ai/trading-intent}{github.com/galpha-ai/trading-intent}）应被理解为这一架构中 contract layer 的当前参考实现：schema definition、template generation、parsing、validation 与 dispatch。也正因为如此，parser、schema、validator、simulator、adapter 与 agent 可以独立演化，但仍通过共享表示保持兼容。
